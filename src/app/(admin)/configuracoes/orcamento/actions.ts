...
/**
 * Distribui valor anual igualmente entre os 12 meses.
 * Usado pelo botão "Distribuir igualmente" na UI.
 */
export async function distributeBudgetEvenlyAction(formData: FormData): Promise<void> {
  const dim = formData.get('dimension') as Dimension;
  const groupId = String(formData.get('group_id') ?? '');
  const fkId = String(formData.get('fk_id') ?? '');
  const fiscalYear = Number(formData.get('fiscal_year') ?? 0);
  const total = Number(formData.get('amount_annual') ?? 0);
  const id = formData.get('id') ? String(formData.get('id')) : null;

  if (!groupId || !fkId || !fiscalYear || total < 0) throw new Error('Dados inválidos');

  const supabase = await createClient();
  const table = tableFor(dim);
  const fkCol = fkColFor(dim);

  // Rateia exato: 11 meses recebem floor(total/12), dezembro recebe o resto.
  const per = Math.floor((total / 12) * 100) / 100;
  const monthly: Record<string, number> = {};
  let acc = 0;
  for (let m = 1; m <= 11; m++) {
    monthly[String(m)] = per;
    acc += per;
  }
  monthly['12'] = Math.round((total - acc) * 100) / 100;
  ...
  if (id) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ({ error } = await (supabase as any).from(table).upsert(
      {
        group_id: groupId,
        [fkCol]: fkId,
        fiscal_year: fiscalYear,
