...
type StatusOpt = 'pending' | 'received' | 'cancelled' | 'all';

export function ArFiltersForm({
  empresas,
  orgFilter,
  statusFilter,
  fromFilter,
  ...
  return (
    ...
    >
      ...
      <div>
        <label className="block text-xs uppercase text-neutral-500 mb-1">Vencimento de</label>
        <input
          name="from"
          type="date"
      ...
      {countLabel && <span className="ml-auto text-xs text-neutral-500">{countLabel}</span>}
    </form>
  );
}
