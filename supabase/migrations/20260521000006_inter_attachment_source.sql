-- ============================================================
-- 20260521000006_inter_attachment_source.sql
-- ------------------------------------------------------------
-- Adiciona 'inter' como origem válida de accounts_payable_attachments.
-- O comprovante de pagamento puxado da API do Banco Inter é anexado
-- na CAP com source='inter' (kind='receipt').
-- ============================================================
...
ALTER TABLE public.accounts_payable_attachments
  ADD CONSTRAINT accounts_payable_attachments_source_check CHECK (source IN (
    'manual', 'ai_import', 'supplier_portal', 'bling', 'focus', 'inter'
  ));
